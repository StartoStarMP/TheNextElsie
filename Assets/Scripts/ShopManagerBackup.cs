...
public class ShopManagerBackup : MonoBehaviour
{
    ...
    public Button[] discountButtons;
    public GameObject[] discountLockOverlays;
    //QUALITY
    public Image itemQuality;
    public Button[] qualityButtons;
    public Sprite[] qualitySprites;
    public GameObject[] qualityLockOverlays;
    //STYLES
    ...
    public void UpdateItemUpgrades()
    {
        ...
        for(int i = 0; i < discountLockOverlays.Length; i++)
        {
            if (i < discountTier)
            {
                discountLockOverlays[i].gameObject.SetActive(false);
                discountButtons[i].onClick.RemoveAllListeners();
            }
            else if (i == discountTier)
            {
                discountLockOverlays[i].gameObject.SetActive(true);
                upgradeButton.onClick.RemoveAllListeners();
                discountButtons[i].onClick.AddListener(delegate { upgradeButton.onClick.AddListener(delegate { UpgradeDiscount(); }); });
            }
            else if (i > discountTier)
            {
                discountLockOverlays[i].gameObject.SetActive(true);
        ...
        for (int i = 0; i < qualityLockOverlays.Length; i++)
        {
            if (i < qualityTier)
            {
                qualityLockOverlays[i].gameObject.SetActive(false);
                qualityButtons[i].onClick.RemoveAllListeners();
            }
            else if (i == qualityTier)
            {
                qualityLockOverlays[i].gameObject.SetActive(true);

                upgradeButton.onClick.RemoveAllListeners();
                qualityButtons[i].onClick.AddListener(delegate { upgradeButton.onClick.AddListener(delegate { UpgradeQuality(); }); });
            }
            else if (i > qualityTier)
            {
                qualityLockOverlays[i].gameObject.SetActive(true);
        ...
        foreach(Button styleButton in styleButtons)
        {
            styleButton.gameObject.SetActive(false);
        }
        ...
        for (int i = 0; i < displayedItemInfo.itemStyles.Count; i++)
        {
            ...
            {
                styleLockOverlays[i].gameObject.SetActive(false);
                styleButtons[i].onClick.RemoveAllListeners();
            }
            else
            {
                styleLockOverlays[i].gameObject.SetActive(true);
                int x = i;
                upgradeButton.onClick.RemoveAllListeners();
                styleButtons[i].onClick.AddListener(delegate { upgradeButton.onClick.AddListener(delegate { UnlockStyle(x); }); });
            }
        }
    }
